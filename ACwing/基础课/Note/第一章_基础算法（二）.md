...
![](\pictures\2022-11-29-17-08-36.png)
...
### 一维前缀和
假设有一个数组：$a_{1}，a_{2}，a_{3}，a_{4}，a_{5}，…，an$（注意，下标从1开始）
...
前缀和 $S_{ij}$ 表示点 $a_{ij}$ 及其左上角区域的所有数的和。
...
![](\pictures\2022-11-30-15-59-40.png)
...
数组b的构造
$b_{1} = a_{1}，b_{2} = a_{2} - a_{1}，b_{3} = a_{3} - a_{2}，…，b_{n} = a_{n} - a_{n-1}$
...
因此在输入数组a时，可以先假想数组a和数组b的全部元素都是0。然后每次进行一次插入操作（指的是对数组a的[l, r]区间的每个数加上常数C），比如
对a数组区间[1,1]，加（插入）常数$a_{1}$；对区间[2,2]，加常数$a_{2}$，…，这样在输入数组a的同时，就能够快速构造出其差分数组b
...
![](pictures/Snipaste_2022-12-01_10-35-52.png)
